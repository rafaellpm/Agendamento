...
uses
  UCadProduto, UCadForn, UCadAgendamento, UCadProced;
...
begin
  FrmCadProced := TFrmCadProced.Create(self);
  try
    FrmCadProced.ShowModal;
  finally
    FreeAndNil(FrmCadProced);
  end;
end;
...
begin
  try
    ...
    ShowMessage('Selecione o agendamento');
  end;
end;
...
begin
  ...
  begin
    ...
    Remarcar1.Enabled := False
  end

  else if CBSituacao.Text = 'CONCLUIDO' then
  begin
  ...
  else if CBSituacao.Text = 'CANCELADO' then
  begin
    FinalizarAtendimento1.Enabled := False;
    Remarcar1.Enabled := False;
    Iniciar1.Enabled := False;
  ...
  else if CBSituacao.Text = 'AGENDADO' then
  begin
    FinalizarAtendimento1.Enabled := False;
    Remarcar1.Enabled := True;
